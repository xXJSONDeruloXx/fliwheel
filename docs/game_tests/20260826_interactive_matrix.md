...
| `55555` | Bejeweled | Classic mode reaches the tutorial and live 8×8 board; the deterministic Classic replay enters the guest swap path, changes/refills tiles, and opens the score-bar overlay; the current-tree replay maps the known sources and emits `swap.wav`, `gotset.wav`, `gemongem.wav`, and `combo2.wav`; a positive menu-wheel step selects Action mode, whose headed route accepts a first board swap, emits `swap.wav`/`gotset.wav`/`gemongem.wav`, shows score `50` and `Well done!`, then returns to a live 8×8 board with the vertical timer bar; a 4,000-frame idle Action replay shows the timer gauge decrementing without a fatal transition | Classic live match and one accepted Action-mode match plus timer progression verified; guest WAV routing and headed sink dispatch partial; multiple Action moves, timeout/game-over, save, and mixer coverage remain | Reproduce multiple accepted Action-mode moves and extend the timer run to the time-up/game-over boundary, then verify persistence and mixer behavior |
