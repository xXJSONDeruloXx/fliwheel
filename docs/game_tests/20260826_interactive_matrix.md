...
| `55555` | Bejeweled | Normalized wheel input reaches the tutorial and live 8×8 board; the historical deterministic `[8,6]` to `[8,7]` swap enters the guest swap path, displays “EXCELLENT!”, changes/refills tiles, and opens the score-bar overlay; the current exact replay maps 27/27 WAV sources, emits `swap.wav` plus `bad.wav`, and the headed sink accepts 81/81 queued events; the runner maps arrow keys to the four tap quadrants | Single live match verified; guest WAV routing and headed sink dispatch partial; mode/match-audio/save coverage remains | Reproduce the valid match on the current board, then compare mixer behavior and verify persistence |
