...
| `AAAAA` | PAC-MAN | Default path reaches name entry, the guest informational prompt, main menu, and the `START GAME / MODE / STAGE / BACK` screen; the title-scoped TGA completion probe reaches a rendered Stage 1 maze, moves Pac-Man, advances the score 0→30→40 through frame 1048 with no fatal signature, and the headed replay records 12 mapped WAV events accepted by the sink; no request for the executable's missing `tex_menu.tga` | Diagnostic gameplay/audio path; default callback contract, physical mixer parity, collisions/lives, and persistence remain open | Make the proven TGA completion contract default, then verify pause/collision/lives, physical audio/mixer behavior, persistence, and long-run play |
