...
| `55555` | Bejeweled | Classic mode reaches the tutorial and live 8×8 board; the deterministic Classic replay enters the guest swap path, changes/refills tiles, and opens the score-bar overlay; the current-tree replay maps the known sources and emits `swap.wav`, `gotset.wav`, `gemongem.wav`, and `combo2.wav`; a positive menu-wheel step selects Action mode, whose headed route reaches a live 8×8 board with the vertical timer bar and dispatches `swap.wav`/`bad.wav`; a 4,000-frame idle Action replay shows the timer gauge decrementing without a fatal transition | Classic live match and Action-mode board/timer progression verified; guest WAV routing and headed sink dispatch partial; accepted Action match, timeout/game-over, save, and mixer coverage remain | Reproduce an accepted Action-mode match and extend the timer run to the time-up/game-over boundary, then verify persistence and mixer behavior |
