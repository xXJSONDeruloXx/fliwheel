...
| `14004` | Ms. PAC-MAN | Normal path completes the four measured async stages, reaches name entry/main menu/Play Game/tutorial, then reaches a controllable Stage 1 maze and advances the score 0→160 with raw clickwheel quadrant input; the default replay is clean through frame 899; a follow-up maps all 20 WAV sources and dispatches 18 observed events through the headed sink | Normal-path Stage 1 and headed WAV dispatch verified; collisions/lives, persistence, and long-run play remain open | Verify collision/life behavior, persistence, and long-run play |
