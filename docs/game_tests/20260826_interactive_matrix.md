...
| `55555` | Bejeweled | Classic mode reaches the tutorial and live 8×8 board; the deterministic Classic replay enters the guest swap path, changes/refills tiles, and opens the score-bar overlay; the current-tree replay maps the known sources and emits `swap.wav`, `gotset.wav`, `gemongem.wav`, and `combo2.wav`; a positive menu-wheel step selects Action mode, whose headed route accepts two board swaps, emits the expected `swap.wav`/`gotset.wav`/`gemongem.wav` sequence plus `combo2.wav` on the second match, shows score `50` and `Well done!` twice, then returns to live 8×8 boards with the vertical timer bar; a 4,000-frame idle Action replay shows the timer gauge decrementing without a fatal transition | Classic live match and two accepted Action-mode matches plus timer progression verified; guest WAV routing and headed sink dispatch partial; further Action moves, timeout/game-over, save, and mixer coverage remain | Reproduce further accepted Action-mode moves and extend the timer run to the time-up/game-over boundary, then verify persistence and mixer behavior |
| `66666` | Tetris | Fresh controlled evidence proves the board, pause/resume, opposite side-button rotations, signed wheel movement, hard drop, a guest row clear, indexed `Menu.wav`/`Move.wav`/`Drop.wav`/`Lock.wav`/`Clear.wav` events, headed sink playback including `Clear.wav`, clean wall-adjacent rotations at both outer columns, and accepted plus collision-blocked rotations against a guest-built stack | Best current target; core gameplay/audio path and collision-dependent blocked-rotation path verified, interactive partial, not complete | Prove an unambiguous kick translation/formal kick table, then finish piece sequencing/scoring, persistence, long-run visual parity, and physical mixer parity |
| `77777` | Mahjong | Opt-in RLB stream now reaches the reference 552x110 RGBA5551 and 40x36 RGB565 title resources; readable title art is captured, while menu/board play remains unverified | Readable title art + stream partial | Drive the post-title state, then verify tile input, audio, and save/persistence |
...
| `99999` | Cubis 2 | Follow-up live-GL captures reach the readable 49-draw main menu and the complete underwater/isometric level scene; wheel/select actions move the board state, all 21 effect WAV sources map to handles, and a headed run logs resolved sink playback | Main menu and board rendering verified; effect audio path verified; matching, music, save, persistence, and full play remain open | Reproduce completed cube matches across levels, then verify pause/return, music/mixer behavior, save, and persistence |
