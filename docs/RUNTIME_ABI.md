...
| `GL:38` | `DrawElements(mode, count, GL_UNSIGNED_SHORT, indices_ptr)`: `r2=0x1403`, `r3=index_ptr`; `mode=0x5` is an indexed triangle strip and `mode=0x7` is an indexed GL_QUADS batch. Used by engines that emit NO `GL:158` begin; the first `DrawElements` is the practical frame begin. | Sims Bowling/Pool, Sudoku, Royal Solitaire (`0x5`); PAC-MAN (`0x7`) |
