#!/usr/bin/env bash
#
# scripts/tetris.sh — launch the Tetris iPod game in headed mode with all the
...
# Path to the Tetris bundle (Games_RO/66666). Override with $TETRIS_BUNDLE.
DEFAULT_BUNDLE="${TETRIS_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/66666}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set TETRIS_BUNDLE or pass a path: ./scripts/tetris.sh /path/to/66666" >&2
    exit 1
fi
