#!/usr/bin/env bash
#
# scripts/tetris.sh — launch the Zuma iPod game in headed mode with all the
...
# Path to the Zuma bundle (Games_RO/44444). Override with $TETRIS_BUNDLE.
DEFAULT_BUNDLE="${TETRIS_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/44444}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set TETRIS_BUNDLE or pass a path: ./scripts/tetris.sh /path/to/44444" >&2
    exit 1
fi
