#!/usr/bin/env bash
#
# scripts/zuma.sh — launch the Zuma iPod game in headed mode with all the
# experimental live GL HLE diagnostics enabled.
#
# Usage:
#   ./scripts/zuma.sh                 # default: build + headed run, log to /tmp
#   ./scripts/zuma.sh --timeout 15    # auto-terminate after 15 seconds
#   ./scripts/zuma.sh --no-capture    # skip PPM frame captures
#   ./scripts/zuma.sh --no-build      # skip the cargo build step
#   ./scripts/zuma.sh --headless      # no window (for CI / quick checks)
#   ./scripts/zuma.sh --verbose       # debug-level logging
#   ./scripts/zuma.sh --dump 30       # dump first 30 presented frames as PPM
#   ./scripts/zuma.sh -- /path/to/bundle   # override bundle dir (must be last)
#
# Logs are written to: /tmp/zuma_run_YYYYMMDD_HHMMSS.log
# Captures (if enabled): /tmp/zuma_capture_YYYYMMDD_HHMMSS/
#
...
# Path to the Zuma bundle (Games_RO/44444). Override with $ZUMA_BUNDLE.
DEFAULT_BUNDLE="${ZUMA_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/44444}"
BUNDLE="$DEFAULT_BUNDLE"

# Where to put logs and captures.
LOG_DIR="${ZUMA_LOG_DIR:-/tmp}"
CAPTURE_ROOT="${ZUMA_CAPTURE_ROOT:-/tmp}"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            if [[ "$BUNDLE" == "$DEFAULT_BUNDLE" && -z "${ZUMA_BUNDLE:-}" ]]; then
                BUNDLE="$1"; shift
            else
                EXTRA_ARGS+=("$1"); shift
            fi
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set ZUMA_BUNDLE or pass a path: ./scripts/zuma.sh /path/to/44444" >&2
    exit 1
fi
...
LOG_FILE="$LOG_DIR/zuma_run_${STAMP}.log"
CAPTURE_DIR="$CAPTURE_ROOT/zuma_capture_${STAMP}"
...
if [[ "$DO_BUILD" -eq 1 ]]; then
    ...
    BUILD_LOG="$(mktemp -t zuma_build.XXXXXX)"
